...
public class LocationGrpcService(ILogger<LocationGrpcService> logger, IOptions<GlobalConfig> config) : ILocationService
{
    ...
    public async Task<IList<Models.Location>> GetLocationsAsync(LocationFiltersDto filters, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return locations
                .Select(loc => new Models.Location
                {
                    Id = loc.Id,
                    Name = loc.Name,
