...
) : IAuthenticationService
{
    public async Task<SessionInfoDto> SignInAsync(string email, string password, string fingerprint, string userAgent)
    {
        ...
        if (!passwordHasherService.VerifyPassword(password, authData.Hash, authData.Salt))
        {
            throw new Exception("Wrong email or password");
        }
        ...
        var accessToken = jwtTokenService.GenerateAccessToken(user, session);
    ...
    public async Task<SessionInfoDto> RefreshTokensAsync(string refreshToken, string fingerprint)
    {
        ...
        var accessToken = jwtTokenService.GenerateAccessToken(user, newSession);
