...
public record ReleaseByCodeCommand(uint IssuerId, Guid InstanceCode, Guid LocationCode) : IRequest;

public class ReleaseByCodeCommandValidator : AbstractValidator<ReleaseByCodeCommand>
{
    public ReleaseByCodeCommandValidator()
    {
        RuleFor(x => (int)x.IssuerId).NotEqual(0).WithMessage("Instance ID must not be zero.");
...
public class ReleaseByCodeCommandHandler(IReservationService reservationsService) : IRequestHandler<ReleaseByCodeCommand>
{
    ...
    {
        await reservationsService.ReleaseAsync(request.IssuerId, request.InstanceCode, request.LocationCode, cancellationToken);
    }
}